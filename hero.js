...
function updateSlide(number) {
    ...
    if (number > int_size - 1) { //If at end of slides, reset to beginning//
        int_slide = 0
    } else if (number < 0) {
        int_slide = int_size - 1;
    } else { //Otherwise go to next slide in list//
        int_slide = number;
    }
    list_slides[int_slide].classList.add('visible'); //Make new slide visible
